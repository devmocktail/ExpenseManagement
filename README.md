...
against an ASP.NET Core API on SQL Server.

> **Status: in development.** The backend is built and verified end to end
> against a real SQL Server instance. The mobile app typechecks clean and has
...
| Database | SQL Server 2025 (LocalDB in development) |
...
- **SQL Server** — LocalDB is enough and ships with the SQL Server Express installer
...
- **Money is `decimal` end to end**, mapped to `decimal(18,2)`. No float, ever.
...
- **No automated tests yet.** The test projects exist and are wired into the
  solution but are empty. Verification so far has been a scripted end-to-end run
  against a live API (37 checks covering user isolation, token rotation and
  reuse detection, money precision, LIKE escaping and pagination limits) plus a
  browser-driven pass over the mobile flows. That is real evidence, but it is
  not a suite that runs in CI.
...
- **Docker is unverified.** Docker is not installed on the development machine,
  so any compose file here has been written but never run.
